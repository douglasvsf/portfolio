 ...
 */
export function TrackList({ tracks, dict, locale }: { tracks: SpotifyTrack[]; dict: SpotifyDictionary; locale: Locale }) {
  const t = dict.tracks;
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              ...
              {hasAlbum && <TableHead>{t.album}</TableHead>}
              {hasPlays && <TableHead className="text-right">{t.plays}</TableHead>}
              {hasPopularity && <TableHead className="text-right">{t.popularity}</TableHead>}
              {hasDuration && (
                <TableHead className="text-right">
                  <Clock className="ml-auto size-(--size-icon-sm)" aria-label={t.duration} />
                </TableHead>
              )}
            </TableRow>
          </TableHeader>
          <TableBody>
            {tracks.map((track, index) => (
              <TableRow key={track.id} data-testid="track-row">
                ...
                <TableCell>
                  <CoverArt images={track.album.images} seed={track.album.id} alt={fmt(t.coverAlt, { name: track.album.name || track.name })} size={40} />
                </TableCell>
                ...
                {hasPlays && (
                  <TableCell className="text-right font-mono tabular-nums text-muted-foreground">{track.playcount?.toLocaleString(locale) ?? "—"}</TableCell>
                )}
                {hasPopularity && (
      ...
      <ol className="flex flex-col divide-y divide-border md:hidden">
        {tracks.map((track, index) => (
          <li key={track.id} data-testid="track-card" className="flex items-center gap-3 px-1 py-2.5">
            ...
            <CoverArt images={track.album.images} seed={track.album.id} alt={fmt(t.coverAlt, { name: track.album.name || track.name })} size={44} />
            ...
            <span className="shrink-0 font-mono text-caption tabular-nums text-muted-foreground">
              {track.playcount !== undefined ? plural(t.playsCount, track.playcount, locale) : track.duration_ms > 0 ? formatDuration(track.duration_ms) : null}
            </span>
          </li>
        ))}
      </ol>
